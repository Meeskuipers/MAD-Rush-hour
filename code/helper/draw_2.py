...
def draw(cars, colours, canvas, size, exp):
    canvas.create_rectangle(0,0,exp+exp*size,exp+exp*size, outline="black", fill="black")
    for i in range(size):
        for x in range(size):
            canvas.create_rectangle(5+(i*exp),5+(x*exp),exp+(i*exp),exp+(x*exp), outline="white", fill="white")
    ...
    for car in cars:
        ...
        canvas.create_rectangle(5+(int(cars[car][0][1])*exp),5+(int(cars[car][0][0])*exp),exp+(int(cars[car][-1][1])*exp), exp+(int(cars[car][-1][0])*exp), outline=colour, fill=colour)
